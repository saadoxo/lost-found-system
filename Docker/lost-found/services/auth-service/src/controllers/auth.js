...
exports.register = async (req, res) => {
  ...
  try {
    ...
    console.log(JSON.stringify({ level: 'info', event: 'user_registered', userId: result.rows[0].id }));
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      // Run a dummy hash to keep response time consistent regardless of whether the user exists
    ...
    if (!valid) {
      console.log(JSON.stringify({ level: 'warn', event: 'login_failed', email, ip: req.ip }));
      return res.status(401).json({ error: 'Invalid credentials' });
    }
    ...
    console.log(JSON.stringify({ level: 'info', event: 'login_success', userId: user.id }));
...
exports.logout = async (req, res) => {
  try {
    ...
    console.log(JSON.stringify({ level: 'info', event: 'logout', userId: req.user.sub }));
