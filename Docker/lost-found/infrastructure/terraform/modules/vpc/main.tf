...
resource "aws_eip" "nat" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.project}-eip-nat-${count.index}-${var.environment}"
  })
...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.project}-nat-${var.availability_zones[count.index]}-${var.environment}"
  })
...
resource "aws_route_table" "private" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.project}-rt-private-${var.availability_zones[count.index]}-${var.environment}"
  })
}
